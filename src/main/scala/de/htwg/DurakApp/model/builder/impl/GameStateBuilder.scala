...
import de.htwg.DurakApp.model.{Card, Player, Rank, Suit, GameState}
...
object GameStateBuilder {
  def apply(gamePhases: GamePhases): GameStateBuilder =
    new GameStateBuilderImpl(gamePhases = gamePhases)
}
...
private[model] case class GameStateBuilderImpl(
  ...
  def build(): Try[GameState] = {
    Try {
      val defaultTrumpCard = trumpCard.getOrElse(Card(Suit.Hearts, Rank.Six))
