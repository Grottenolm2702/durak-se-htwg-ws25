...
/** Controller trait for managing game flow and state transitions.
  ...
  * manages undo/redo operations, and notifies observers (views) of state changes.
  * It implements the Observable pattern to allow multiple views to react to
  * game state updates.
  *
...
trait Controller:
  ...
  /** Processes a player action and updates the game state.
    *
    * This method is the main entry point for all player interactions.
    * It validates the action, executes it through the command pattern,
    * handles phase transitions, and notifies all observers.
    *
    * @param action The player action to process (e.g., PlayCardAction, PassAction)
    * @return The updated game state after processing the action
    */
  def processPlayerAction(action: PlayerAction): GameState
  
  /** Undoes the last action.
    *
    * Reverts the game state to the previous state before the last action.
    * Uses the command pattern to restore state from the undo stack.
    *
    * @return Some(GameState) if undo was successful, None if no actions to undo
    */
  def undo(): Option[GameState]
  
  /** Redoes the last undone action.
    *
    * Reapplies the most recently undone action. Only works if undo was
    * called before and no new actions were performed since.
    *
    * @return Some(GameState) if redo was successful, None if no actions to redo
    */
  ...
  /** Returns a string representation of the current game status.
    ...
    *
    * @return A status string describing the current game state
    */
...
object Controller:
  /** Creates a new Controller.
    ...
    * @param undoRedoManager The manager for undo/redo command history
