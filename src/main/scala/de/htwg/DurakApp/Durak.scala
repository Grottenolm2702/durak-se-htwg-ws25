...
def initDeck() : List[Card] =
    ...
    val trumpShuffledDeck = shuffledDeck.map(card => card.copy(isTrump = card.suit == trump))
...
def initPlayerList(deck : List[Card]): (List[Player], List[Card]) = 
    ...
    val (players, remainingDeck) =(1 to numPlayers).foldLeft((List.empty[Player], deck)) {
