  ...
  "A GamePhaseInputHandler" when {
    ...
    "in SetupPhase or AskPlayerCountPhase" should {
      "return SetPlayerCountAction for valid integer input" in {
        handler.handleRequest("3", gameStateWithPhase(SetupPhase)) should be(
          SetPlayerCountAction(3)
        )
        handler.handleRequest(
          "2",
          gameStateWithPhase(AskPlayerCountPhase)
        ) should be(SetPlayerCountAction(2))
      }

      "return InvalidAction for invalid integer input" in {
        handler.handleRequest("abc", gameStateWithPhase(SetupPhase)) should be(
          InvalidAction
        )
        handler.handleRequest(
          "",
          gameStateWithPhase(AskPlayerCountPhase)
        ) should be(InvalidAction)
      }
    }
    ...
    "in AskPlayerNamesPhase" should {
      "return AddPlayerNameAction for any string input" in {
        handler.handleRequest(
          "Ronny",
          gameStateWithPhase(AskPlayerNamesPhase)
        ) should be(AddPlayerNameAction("Ronny"))
        handler.handleRequest(
          "  Anna  ",
          gameStateWithPhase(AskPlayerNamesPhase)
        ) should be(AddPlayerNameAction("Anna"))
        handler.handleRequest(
          "",
          gameStateWithPhase(AskPlayerNamesPhase)
        ) should be(AddPlayerNameAction(""))
      }
    }
    ...
    "in AskDeckSizePhase" should {
      "return SetDeckSizeAction for valid integer input" in {
        handler.handleRequest(
          "24",
          gameStateWithPhase(AskDeckSizePhase)
        ) should be(SetDeckSizeAction(24))
        handler.handleRequest(
          "36",
          gameStateWithPhase(AskDeckSizePhase)
        ) should be(SetDeckSizeAction(36))
      }
    ...
    "in AskPlayAgainPhase" should {
      "return PlayAgainAction for 'yes' (case-insensitive)" in {
        handler.handleRequest(
          "yes",
          gameStateWithPhase(AskPlayAgainPhase)
        ) should be(PlayAgainAction)
        handler.handleRequest(
          "YES",
          gameStateWithPhase(AskPlayAgainPhase)
        ) should be(PlayAgainAction)
