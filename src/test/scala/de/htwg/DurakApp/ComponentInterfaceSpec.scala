...
/**
 * Component Interface Tests with Test Doubles (Mocks, Stubs, Spies)
 * All tests run ONLY through interfaces
 */
class ComponentInterfaceSpec extends AnyWordSpec with Matchers {
  ...
  /**
   * Mock Controller - Test Double that simulates controller behavior
   */
  class MockController(initialState: GameState) extends ControllerInterfaceTrait with Observable {
    private var _gameState: GameState = initialState
    private val _actionHistory = scala.collection.mutable.ListBuffer.empty[PlayerAction]
    private val _undoStack = scala.collection.mutable.ListBuffer.empty[GameState]
    private val _redoStack = scala.collection.mutable.ListBuffer.empty[GameState]
      ...
      val newState = action match {
        case SetPlayerCountAction(count) =>
          _gameState.copy(
            ...
            gamePhase = if (count >= 2 && count <= 6) AskPlayerNamesPhase else _gameState.gamePhase,
            lastEvent = if (count >= 2 && count <= 6) Some(GameEvent.AskPlayerNames) else Some(GameEvent.SetupError)
          )
        case AddPlayerNameAction(name) =>
          ...
          _gameState.copy(
            ...
            gamePhase = if (newNames.length == expectedCount) AskDeckSizePhase else _gameState.gamePhase
          )
  ...
  /**
   * Mock View - Test Double that tracks update calls
   */
  ...
  /**
   * Spy Controller - Wraps real controller and counts calls
   */
  class SpyController(real: Controller) extends ControllerInterfaceTrait with Observable {
  ...
  /**
   * Stub GameState - Returns hardcoded test data
   */
  object StubGameState {
    def setupStub(): GameState = GameStateBuilder().withGamePhase(SetupPhase).build()
  ...
  /**
   * Fake Controller - Simplified implementation for testing
   */
  class FakeController extends ControllerInterfaceTrait with Observable {
    ...
    override def processPlayerAction(action: PlayerAction): GameState = {
      ...
      action match {
        case SetPlayerCountAction(count) if count >= 2 =>
          _state = _state.copy(setupPlayerCount = Some(count), gamePhase = AskPlayerNamesPhase)
        case _ => 
      }
      notifyObservers
      _state
    ...
    override def undo(): Option[GameState] = if (_undoAvailable) Some(_state) else None
  ...
  "A SpyController through ControllerInterface" should {
    ...
    "count processPlayerAction calls" in {
      val realController = new Controller(StubGameState.setupStub(), UndoRedoManager())
